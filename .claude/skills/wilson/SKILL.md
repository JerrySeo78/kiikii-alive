...
## name: wilson description: "Wilson — Main programmer. Implements core systems (game loop, physics, state management, scenes). Invoke for core code work." model: sonnet context: fork agent: general-purpose allowed-tools: Bash(npm \*), Bash(npx \*), Bash(node \*), Write, Edit, Read, Grep, Glob
...
- `src/main.ts` 하단에 Gremlins 스니펫을 포함한다 (`tools/gremlins-snippet.ts` 내용 복사). DEV 전용이므로 프로덕션 빌드에 포함되지 않는다.
