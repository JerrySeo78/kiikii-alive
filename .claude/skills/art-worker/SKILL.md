...
### Step 2-6: 애니메이션 스프라이트 — Veo 파이프라인 (해당 시)

**트리거 조건:** 걷기/달리기 등 팔다리 교차(contralateral movement)가 필요한 3프레임 이상 애니메이션

Gemini 직접 생성으로는 팔다리 교차 규칙이 불안정하다. 이런 경우 Veo 영상 기반 파이프라인을 사용한다.

**판단 기준:**
| 상황 | 방식 |
|------|------|
| 정지 포즈, idle blink, 단순 2프레임 | Gemini 직접 생성 |
| 걷기/달리기 3프레임 이상 | **Veo 파이프라인** |

**실행:**
```bash
# 1. 템플릿 복사
cp templates/scripts/sprite-anim-veo.mjs scripts/generate-[animName].mjs

# 2. 파일 상단 CONFIG 블록 수정
#    charPath, charName, animName, standingPrompt, videoPrompt

# 3. 실행 (~$0.20/애니메이션)
GOOGLE_API_KEY=$GOOGLE_API_KEY node scripts/generate-[animName].mjs

# 프레임 타이밍 재조정 시 (영상 재생성 불필요)
node scripts/generate-[animName].mjs --extract-only
```

**상세:** `.reference/coding-strategies/veo-sprite-pipeline_20260430.md`
...
나머지 에셋 일괄 생성 (레퍼런스 체이닝, 비율 적용)
    ↓
[애니메이션 스프라이트] Veo 파이프라인 (걷기/달리기 해당 시)
    ↓
